...
def to_code(config):
    ...
    if CONF_SNIFF_AFTER_COMMAND_SENT_X_TIMES in config:
        cg.add(
            var.set_sniff_after_command_sent_x_times(
                config[CONF_SNIFF_AFTER_COMMAND_SENT_X_TIMES]
            )
        )
    ...
    for conf in config.get(CONF_ON_REMOTE_CLICK, []):
        ...
        yield auto.build_automation(
            trigger, [(cg.uint16, "address"), (cg.uint8, "command")], conf
        )
